...
const syncTable = Effect.fn("syncTable")(function* (modelId: string) {
  const sql = yield* SqlClient.SqlClient;
...
export const listFields = Effect.fn("listFields")(function* (modelId: string) {
  const sql = yield* SqlClient.SqlClient;
...
export const createField = Effect.fn("createField")(function* (modelId: string, body: CreateFieldInput) {
  const sql = yield* SqlClient.SqlClient;
  const models = yield* sql.unsafe<{ id: string }>("SELECT id FROM models WHERE id = ?", [modelId]);
  if (models.length === 0) return yield* new NotFoundError({ entity: "Model", id: modelId });
...
export const updateField = Effect.fn("updateField")(function* (fieldId: string, body: UpdateFieldInput) {
  const sql = yield* SqlClient.SqlClient;
...
export const deleteField = Effect.fn("deleteField")(function* (fieldId: string) {
  const sql = yield* SqlClient.SqlClient;
  const fields = yield* sql.unsafe<FieldRow>("SELECT * FROM fields WHERE id = ?", [fieldId]);
  if (fields.length === 0) return yield* new NotFoundError({ entity: "Field", id: fieldId });
