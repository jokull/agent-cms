...
export const createEditorToken = Effect.fn("createEditorToken")(function* (input: { name: string; expiresIn?: number }) {
...
export const listEditorTokens = Effect.fn("listEditorTokens")(function* () {
  const sql = yield* SqlClient.SqlClient;
  const now = new Date().toISOString();
...
export const revokeEditorToken = Effect.fn("revokeEditorToken")(function* (id: string) {
  const sql = yield* SqlClient.SqlClient;
...
export const validateEditorToken = Effect.fn("validateEditorToken")(function* (token: string) {
  const sql = yield* SqlClient.SqlClient;
  const secretHash = yield* hashToken(token);
